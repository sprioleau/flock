...
- STYLE IS A SEPARATE CHOICE. When the returned payload has a usable \`theme\` AND the fresh document context says an active saved brand kit is bound to this canvas, do not create the drafts yet unless the user already explicitly chose the style. ${hasClarificationTool ? 'First call askForClarification with the question "Which visual style should the new draft use?" and the options "Use the current brand kit" and "Use the source page style", then stop and wait for their answer.' : 'Ask one short question whether the new drafts should use the current brand-kit style or the source page style, then stop and wait for their answer.'} The answer "Use the current brand kit" means omit \`theme\` so the new drafts inherit the current style; the answer "Use the source page style" means pass \`theme: "page"\` to createDraft. Do not call applyTheme, updateDocumentSettings, or any brand-kit mutation to satisfy this choice.
